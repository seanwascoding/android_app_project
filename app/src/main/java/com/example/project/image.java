...
public class image extends AppCompatActivity {
    ...
    public void post(View view) {
        if(picture_name.getText().toString().isEmpty() || imageView.getDrawable()==null || findViewById(R.id.select_image).isEnabled()){
    ...
    //transfer to gray
    public void gray(View view) throws JSONException {
        if(picture_name.getText().toString().isEmpty() || imageView.getDrawable()==null || findViewById(R.id.post).isEnabled()){
            Toast.makeText(this, "Input the picture name & select photo", Toast.LENGTH_SHORT).show();
        }
        else {
            //fixed
            findViewById(R.id.gray).setEnabled(false);

            //
            JSONObject jsonObject = new JSONObject();
            jsonObject.put(picture_name.getText().toString(), "1");
    ...
    public void encryption(View view) throws JSONException {
        if(picture_name.getText().toString().isEmpty() || imageView.getDrawable()==null || findViewById(R.id.gray).isEnabled()){
            Toast.makeText(this, "Input the picture name & select photo", Toast.LENGTH_SHORT).show();
        }
        else {
            //fixed
            ...
            //
            JSONObject jsonObject = new JSONObject();
            jsonObject.put(picture_name.getText().toString(), "2");
            String jsonString = jsonObject.toString();
            new PostDate_2().execute(jsonString);
        }
    }
    ...
    public void decrypt(View view) throws JSONException {
        if(picture_name.getText().toString().isEmpty() || imageView.getDrawable()==null || findViewById(R.id.encryption).isEnabled()){
            Toast.makeText(this, "Input the picture name & select photo", Toast.LENGTH_SHORT).show();
        }
        else {
            //fixed
            findViewById(R.id.decryption).setEnabled(false);

            //
            JSONObject jsonObject = new JSONObject();
            jsonObject.put(picture_name.getText().toString(), "3");
            String jsonString = jsonObject.toString();
            new PostDate_2().execute(jsonString);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1 && resultCode == RESULT_OK && data.getData() != null) {
            ...
            String filePath=getRealPathFromUri(image.this,uri);
    ...
    class PostData extends AsyncTask<File, Void, File> {
        ...
        protected File doInBackground(File... file) {
            try {
                ...
                URL url = new URL("http://26.164.96.164:8080/down"); //192.168.1.108
                ...
                try (BufferedReader br = new BufferedReader(new InputStreamReader(client.getInputStream(), "utf-8"))) {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(getApplicationContext(), "Data has been posted to the API.", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
            } catch (Exception e) {
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(getApplicationContext(), "Fail to post the data : " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
            return null;
    ...
    class PostDate_2 extends AsyncTask<String, Void, String>{
        ...
        protected String doInBackground(String... strings){
            try {
                // on below line creating a url to post the data.
                URL url = new URL("http://26.164.96.164:8080/state");
                ...
                try (InputStream inputStream = client.getInputStream()) {
                    ...
                    // 顯示 Toast 訊息
                    runOnUiThread(() -> Toast.makeText(getApplicationContext(), "圖片已接收", Toast.LENGTH_SHORT).show());
                } catch (IOException e) {
                    // 處理錯誤
                    e.printStackTrace();
                }
    ...
    private void connectWebSocket() {
        ...
        webSocketClient = new WebSocketClient(uri_2) {
            ...
            public void onMessage(String message) {
                ...
                runOnUiThread(() -> Toast.makeText(image.this, message, Toast.LENGTH_SHORT).show());
            }
